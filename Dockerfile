...
RUN sudo curl -sL https://github.com/karimra/gnmic/releases/download/v0.20.4/gnmic_0.20.4_Linux_x86_64.rpm -o /tmp/gnmic.rpm && sudo yum localinstall -y /tmp/gnmic.rpm
...
RUN sudo sed -i.orig "s/'Tag ID' : 10,/'Tag ID':10,'VNI':8,'neighbor':10,'IP-address':39,/g" \
